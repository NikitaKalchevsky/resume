...
```
resume/
├── index.html          — разметка + <head> с SEO-метой; тянет styles.css, i18n.js, main.js (main.js динамически import() scene.js)
├── styles.css          — все стили (тёплая тема Organic: токены, компоненты, адаптив, reduced-motion)
├── i18n.js             — словарь переводов window.I18N (EN/UK/RU/ES) для атрибутов data-i18n
├── main.js             — интеракции: переключатель языков (localStorage mk-lang), reveal, счётчики (тонкий пробел), tilt карточек, инициализация 3D-сцены
├── scene.js            — three.js 3D «MK»: export initScene(canvas,host,opts) → {setVariant,destroy}; 3 режима solid/wire/cloud, фолбэк при reduced-motion/no-WebGL
├── index.legacy.html      — бэкап оригинального однофайлового дизайна
├── index.redesign-v1.html — бэкап моего первого многофайлового редизайна (до переноса build)
...
>
> КЭШ: ссылки на `styles.css`/`main.js`/`scene.js` в `index.html` идут с версией `?v=N`. **При правке любого из этих файлов увеличивай `N`** (во всех трёх ссылках), иначе браузер/CDN GitHub Pages отдадут старую версию из кэша, и правки «не появятся» без хард-рефреша. Текущая версия: `v=8`.
...
- **2026-07-31** — Позиционирование: **убраны «веб-приложения»** (пользователь их не делает) по всем 4 языкам. Мета/OG/hero/About/Trajectory/Contact: «websites and web apps» → «websites and online stores» (сайты и интернет-магазины). Карточка About №02 «Web apps» → **«Business websites»** (лендинги и корпоративные сайты). Ключи i18n: lede, aboutLede, b2t/b2d, t1d, contactL в RU/UK/ES. Ассеты `?v=2` (scene.js остался `?v=1` — не менялся).
- **2026-07-30 (3)** — **Деплой: `redesign` смёржен в `main`** (fast-forward) → build-редизайн теперь на GitHub Pages. Добавлен кэш-бастинг `?v=1`: в `index.html` на `styles.css`/`i18n.js`/`main.js`, и в `main.js` на динамический `import('./scene.js?v=1')` (иначе вернувшийся посетитель подтянул бы закэшенный старый `scene.js` без `initScene`, и 3D отвалился бы). Это единственная правка build-файла (`main.js`) — только версия импорта, логика не тронута. При следующих правках css/js — бампать `?v=N`. Smoke-тест ✓ (canvas, `.scenes`, 3 варианта, консоль чистая).
- **2026-07-30 (2)** — **Полный перенос утверждённого макета из `build/build/` в ветку `redesign`** (вариант B). Скопированы БЕЗ ПРАВОК 5 файлов: `index.html`, `styles.css`, `main.js`, `scene.js` + новый `i18n.js`. Моя предыдущая ручная реализация сцены заменена build-версией. Контракт сцены: `scene.js` экспортирует `initScene(canvas, host, opts)` → `{setVariant, destroy}`, подключается из `main.js` через `import('./scene.js')`; кнопки `.scenes [data-variant]` (solid/wire/cloud) + `aria-pressed`; `#hero` = `canvas#scene` + `.hero-scrim` + `.hero-in`. В `<head>` перенесён только favicon; создан `images/og.jpg` (копия `exclusivestyle-home.jpg`) под `og:image`/`twitter:image`. Бэкапы: оригинал — `index.legacy.html`, мой первый редизайн — `index.redesign-v1.html`. Проверено headless: 3D M/K (extrude+bevel+свет), кнопки+aria-pressed, языки+localStorage (`mk-lang`), reduced-motion (canvas 1 кадр, `.scenes` убирается), no-WebGL (canvas+`.scenes` удаляются, hero остаётся тёмной панелью), консоль чистая, реального гориз. скролла на 375px нет (`body{overflow-x:hidden}`; бегущая строка клипается). Build НЕ использует `?v=` — кэш-бастинг на этой ветке отключён (учесть при мерже в main). i18n теперь через `i18n.js`+`window.I18N`, а не словарь в main.js. Расхождения из списка пользователя (лого, 05 Learning, «16 003», Stack 4 колонки, 6 карточек About, курсы, 4 screen-сессии) закрыты самим build. ПОЛНАЯ синхронизация остальных секций гайда — при мерже ветки.
- **2026-07-30** — 3D-hero: буквы M/K **сближены** (позиции ±1.15 → ±0.85, `L_HW` 1.65 → 1.4) и **уменьшены** — в `scene.js resize()` кап по высоте `sV` снижен с ~`(halfH-padV)` до `0.52*halfH`, заполнение зоны `sH` 0.92 → 0.80 (narrow 0.86 → 0.78). Было «слишком крупно» на широких экранах. Ассеты → `?v=8`. (Задачу миграции build/ пока не выполняли — по просьбе только уменьшили текущую сцену.)
- **2026-07-28** — Убраны курсовые сертификаты из секции «Learning» (iTGid ×3, «From 0 to 1» + self-directed edu-item с датами) — читались как junior/red-flag. Вместо списка сертификатов: блок «Currently going deep on» (пилюли `.metric`: Claude API & agents, LLM eval & guardrails, multi-tenant SaaS, payments, i18n) + короткая заметка про self-directed learning; языки без изменений. Реюз существующих компонентов (`.sub-head`/`.metrics`/`.metric`/`.stack-note`), новых стилей нет. Новые i18n-ключи `edu.focus`/`edu.note` (EN/UK/RU/ES). Ассеты → `?v=7`.
...
*Последнее обновление: 2026-07-31 — убраны «веб-приложения», фокус на сайты и интернет-магазины (все 4 языка), ассеты ?v=2. Ранее: новый дизайн Organic: многофайловый статик, 3D-hero «MK» как закруглённая панель с адаптивной сценой (не обрезается на 1440/1024/768/430, статистика вынесена вниз), 7 проектов (флагман Exclusive Style, реальный скрин + og/twitter), i18n на 4 языка. Старый дизайн — index.legacy.html. PDF-хук на http-рендере. Локально открывать только через http (Live Server / npx serve). Ветка `redesign`: перенесён утверждённый макет из build/build дословно (index.html, styles.css, main.js, scene.js + новый i18n.js), сцена — build-версия с контрактом initScene→{setVariant,destroy}, бэкапы index.legacy.html / index.redesign-v1.html, создан images/og.jpg. Смёржено в `main` и задеплоено на Pages; добавлен кэш-бастинг `?v=1` (styles/i18n/main в index.html + import scene.js в main.js). Осталось: живые переводы UK/ES, полная синхронизация остального текста гайда с build.*
