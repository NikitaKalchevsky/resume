...
1. **Exclusive Style, Bespoke Embroidery Atelier** — флагман, `article.project.featured` (акцентная верхняя граница + pill «Flagship»), browser frame · пока плейсхолдер-слот `.img-slot` (нужен скрин `images/exclusivestyle-home.jpg`) · LIVE · E-COMMERCE
...
*Последнее обновление: 2026-07-02 — полная смена дизайна на тему Organic: многофайловый статик (index.html + styles.css + main.js + scene.js), 3D-hero three.js «MK», 7 проектов (флагман Exclusive Style), i18n на 4 языка через data-i18n. Старый дизайн сохранён как index.legacy.html. PDF-хук переписан на http-рендер. Открывать локально только через http (Live Server / npx serve), не через file://. Осталось: скрин Exclusive Style (images/exclusivestyle-home.jpg) и живые переводы UK/ES.*
