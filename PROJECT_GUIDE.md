...
>
> КЭШ: ссылки на `styles.css`/`main.js`/`scene.js` в `index.html` идут с версией `?v=N`. **При правке любого из этих файлов увеличивай `N`** (во всех трёх ссылках), иначе браузер/CDN GitHub Pages отдадут старую версию из кэша, и правки «не появятся» без хард-рефреша. Текущая версия: `v=7`.
